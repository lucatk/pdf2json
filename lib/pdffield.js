...
class PDFField {
    ...
    static getAllFieldsTypes(data) {
		...
		const isFieldRequired = field => {
            return (field.RQ & kFBARequired) ? true : false;
        };

        const getFieldBase = (field, pageIndex) => {
            return {id: field.id.Id, name: field.id.Id, type: field.T.Name, x: field.x, y: field.y, w: field.w, h: field.h, page: pageIndex + 1, readonly: isFieldReadOnly(field), required: isFieldRequired(field), value: field.V || ""};
        };
            ...
            page.Boxsets.forEach( boxsets => {
                if (boxsets.boxes.length > 1) { //radio button
                    boxsets.boxes.forEach( box => {
                        retVal.push({id: boxsets.id.Id, name: field.id.Id, type: "radio", x: field.x, y: field.y, w: field.w, h: field.h, page: pageIndex + 1, readonly: isFieldReadOnly(box), required: isFieldRequired(field),value: box.id.Id});
                    });
                }
                else { //checkbox
                    retVal.push(getFieldBase(boxsets.boxes[0], pageIndex));
                }
            });

            page.Fields.forEach(field => retVal.push(getFieldBase(field, pageIndex)));
